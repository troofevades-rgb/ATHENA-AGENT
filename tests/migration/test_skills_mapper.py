...
def test_imports_hermes_skill_with_full_frontmatter(
    hermes_source: Path, athena_dest: Path, migration_report, hermes_skill_factory
) -> None:
    ...
    import_skills(hermes_source, athena_dest, report=migration_report)

    imported_dir = athena_dest / "skills" / "full"
...
def test_imports_hermes_skill_minimal_frontmatter_with_defaults(
    ...
    fm, _ = parse_frontmatter(athena_dest / "skills" / "bare" / "SKILL.md")
    assert fm.write_origin == "migration"
...
def test_imports_with_references_subdir(
    ...
    import_skills(hermes_source, athena_dest, report=migration_report)
    assert (athena_dest / "skills" / "refsk" / "references" / "notes.md").read_text(
        encoding="utf-8"
    ) == "ref content\n"
...
def test_imports_with_templates_subdir(
    ...
    import_skills(hermes_source, athena_dest, report=migration_report)
    assert (athena_dest / "skills" / "tmpsk" / "templates" / "scaffold.py").exists()
...
def test_imports_with_scripts_subdir(
    ...
    import_skills(hermes_source, athena_dest, report=migration_report)
    assert (athena_dest / "skills" / "scsk" / "scripts" / "go.sh").exists()
...
def test_imports_archived_skill(
    ...
    archived_dest = athena_dest / "skills" / ".archive" / "old-skill"
...
def test_import_conflict_renames_with_from_hermes_suffix(
    ...
    existing = athena_dest / "skills" / "collide"
    ...
    renamed = athena_dest / "skills" / "collide-from-hermes"
...
def test_imported_skill_has_migration_origin(
    ...
    import_skills(hermes_source, athena_dest, report=migration_report)
    fm, _ = parse_frontmatter(athena_dest / "skills" / "origin-test" / "SKILL.md")
    assert fm.write_origin == "migration"
    assert fm.imported_at is not None
...
def test_dry_run_skips_writes(
    ...
    import_skills(hermes_source, athena_dest, report=migration_report, dry_run=True)
    assert not (athena_dest / "skills" / "dryskill").exists()
    assert migration_report.count("imported_skill") == 1
...
def test_malformed_hermes_skill_is_skipped(
    ...
    assert not (athena_dest / "skills" / "bad").exists()
