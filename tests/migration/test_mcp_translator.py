...
def test_copies_mcp_json_verbatim(hermes_source: Path, athena_dest: Path, migration_report) -> None:
    data = {
        "mcpServers": {
    ...
    translate_mcp(hermes_source, athena_dest, report=migration_report)

    out = json.loads((athena_dest / "mcp.json").read_text(encoding="utf-8"))
...
def test_warns_when_no_mcp_json(hermes_source: Path, athena_dest: Path, migration_report) -> None:
    translate_mcp(hermes_source, athena_dest, report=migration_report)
    warnings = migration_report.entries.get("mcp_warning", [])
...
def test_records_error_on_malformed_json(
    ...
    assert not (athena_dest / "mcp.json").exists()
