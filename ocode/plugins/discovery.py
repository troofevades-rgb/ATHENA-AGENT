...
1. ``ocode/plugins/bundled/`` — ships with the package; not user-editable.
2. ``~/.ocode/plugins/`` — user-installed plugins.
...
USER_DIR = Path.home() / ".ocode" / "plugins"
...
def discover(
    *,
    ...
    Arguments default to the package's bundled directory and ``~/.ocode/
