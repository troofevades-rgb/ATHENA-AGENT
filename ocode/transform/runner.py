...
ocode never re-implements training. It builds the dataset, then invokes
...
translating between ocode's terminology (``base_model``, ``learning_rate``,
...
def _default_transform_dir() -> Path:
    ...
    # ocode/transform/runner.py → ocode/transform → ocode → repo root → transform
...
def run_dpo(
    ...
    *,
    ...
    cmd = [
        ...
        "--base", run.base_model,
        "--sft-lora", str(sft_lora_path),
        "--train", str(run.dpo_dataset),
        "--out", str(run.output_dir.with_name(run.output_dir.name + "-dpo")),
        "--epochs", str(max(1, run.epochs // 2)),  # DPO converges faster
        "--lr", str(run.learning_rate / 10),       # smaller LR for DPO
        "--batch", str(run.batch_size),
        "--seq-len", str(run.seq_len),
        *run.extra_args,
    ]
